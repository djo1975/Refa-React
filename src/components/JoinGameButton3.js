...
function JoinGameButton3({ gameId, onJoin }) {
  ...
  const handleJoin = async () => {
    try {
      ...
      const requestData = { game: { player3_id: selectedPlayerId } };
      const updateGameResponse = await axios.put(`http://localhost:3000/games/${gameId}/join/${selectedPlayerId}`, requestData);
  ...
  return (
    <div>
      <button onClick={handleJoin}>Uđi kao levi</button>
      {showModal && (
        <div className="modal">
...
export default JoinGameButton3;
